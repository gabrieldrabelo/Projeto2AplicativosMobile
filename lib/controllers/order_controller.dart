...
class OrderController {
  ...
  Future<List<Order>> getOrders() async {
    ...
    for (var orderMap in orderMaps) {
      ...
      final payments = paymentMaps.map((map) => OrderPayment.fromMap(map)).toList();
  ...
  Future<Order?> getOrderById(int id) async {
    ...
    // Get the order
    final orderMaps = await dbHelper.query('orders', where: 'id = ?', whereArgs: [id]);
  ...
  Future<int> addOrder(Order order) async {
    ...
    if (order.creationDate.isEmpty) {
      order.creationDate = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
    }
  ...
  Future<int> updateOrder(Order order) async {
    ...
    order.lastModified = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
