  ...
  Future<bool> syncUsers() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['dados'] != null) {
          final serverUsers =
              List<User>.from(data['dados'].map((user) => User.fromJson(user)));
          ...
          for (var serverUser in serverUsers) {
            ...
            if (localUser.id == -1) {
              ...
              if (serverUser.lastModified != null &&
                  localUser.lastModified != null) {
              ...
              try {
                ...
                if (postResponse.statusCode == 200 ||
                  ...
                  syncErrors['users']!.add({
                    'id': localUser.id,
                    'error':
                        'Failed to send to server: ${postResponse.statusCode}',
                  });
                }
              } catch (e) {
                syncErrors['users']!.add({
        ...
        if (data['dados'] != null) {
          ...
          for (var serverClient in serverClients) {
            ...
            if (localClient.id == -1) {
              ...
              // Check if server version is newer
              if (serverClient.lastModified != null &&
                  localClient.lastModified != null) {
        ...
        if (data['dados'] != null) {
          ...
          for (var serverProduct in serverProducts) {
            ...
            if (localProduct.id == -1) {
              ...
              // Check if server version is newer
              if (serverProduct.lastModified != null &&
                  localProduct.lastModified != null) {
          ...
          for (var serverOrder in serverOrders) {
            ...
            if (localOrder.id == -1) {
              ...
              await _orderController.addOrder(
                  serverOrder, serverOrder.items, serverOrder.payments);
            } else {
              // Check if server version is newer
              if (serverOrder.lastModified != null &&
                  localOrder.lastModified != null) {
            ...
            if (localOrder.lastModified == null) {
              ...
              try {
                ...
                if (postResponse.statusCode == 200 ||
                    postResponse.statusCode == 201) {
                  // Update lastModified
                  localOrder.lastModified =
                      DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
                  await _orderController.updateOrder(
                      localOrder, localOrder.items, localOrder.payments);
                } else {
